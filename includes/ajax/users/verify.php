...
try {
	...
	if(!isset($_POST['photo']) || is_empty($_POST['photo'])) {
		throw new Exception("Please attach your photo");
	}
	if(!isset($_POST['passport']) || is_empty($_POST['passport'])) {
		throw new Exception("Please attach your Passport or National ID");
	}
	if(!isset($_POST['message']) || is_empty($_POST['message']) ) {
		throw new Exception("Please share why your account should be verified");
	}
	...
	switch ($_GET['node']) {
		case 'user':
			...
			if(!$query) {
				throw new Exception("Your have pending verification request already!");
			}
			break;
